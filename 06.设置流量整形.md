...
根据实际测试，QoS 的处理性能与 RouterOS 网卡的 `Multiqueue` 参数（参阅：[00.PVE下安装系统](./00.PVE下安装系统.md) ）以及 CPU 的单核性能有一定关系。   
...
如果在下行（上行）带宽跑满的时候，出现了如图所示的高时延，并在带宽跑满时持续存在，说明当前下行（上行）带宽上限档位过高。  
...
根据官方文档的说明，虽然在 `Fasttrack` 关闭的情况下支持将 `Simple Queues` 和 `Queue Tree` 同时使用，以达到 “双重流控” 的效果。  

但本文中的示例配置方法并不支持同时使用两种流控，如果你拥有高性能硬件支持 “双重流控” ，欢迎分享你的 QoS 配置方案。  
