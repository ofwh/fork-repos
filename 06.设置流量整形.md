...
```bash
/queue type
add name=cake-rx kind=cake cake-diffserv=diffserv4 cake-flowmode=dual-dsthost cake-memlimit=32M cake-rtt=50ms cake-overhead-scheme=ethernet cake-nat=yes
add name=cake-tx kind=cake cake-diffserv=diffserv4 cake-flowmode=dual-srchost cake-memlimit=32M cake-rtt=50ms cake-overhead-scheme=ethernet cake-nat=yes cake-ack-filter=filter
...
```bash
/queue type
add name=cake-rx kind=cake cake-diffserv=diffserv4 cake-flowmode=triple-isolate cake-memlimit=32M cake-rtt=50ms cake-overhead-scheme=ethernet cake-nat=no
add name=cake-tx kind=cake cake-diffserv=diffserv4 cake-flowmode=triple-isolate cake-memlimit=32M cake-rtt=50ms cake-overhead-scheme=ethernet cake-nat=yes cake-ack-filter=filter
...
```bash
/queue type
add name=fq-codel-rx kind=fq-codel fq-codel-limit=2560 fq-codel-memlimit=16M fq-codel-interval=80ms fq-codel-target=4ms
add name=fq-codel-tx kind=fq-codel fq-codel-limit=2560 fq-codel-memlimit=16M fq-codel-interval=80ms fq-codel-target=4ms
...
对于硬路由 RB750Gr3 ，可仅使用 `Interface Queues` 来达到最大带宽吞吐，并保持相对低延迟：  
