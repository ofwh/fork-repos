...
在设置接口 IPv6 地址时， `pppoe-out1` 接口通常无需分配地址，只需给 `bridge1` 接口分配 IPv6 GUA 公网地址即可。  

对于 IPv6 ULA 私有地址，在以下使用场景中可考虑创建并分配：  

- 仅路由器本身具有 IPv6 GUA 公网地址，内网环境需使用 IPv6 ULA 地址搭配 [NPTv6](https://www.rfc-editor.org/rfc/rfc6296.html) 进行公网访问的场景  

- 内网环境中的部分服务，需固定 IPv6 地址，且 IPv6 LLA 地址无法满足使用需求的场景  

- 内网环境中需要使用 IPv6 ULA 地址进行跨路由器组网的场景  

由于 RouterOS 还未设置 IPv6 防火墙，给内部接口分配 IPv6 GUA 公网地址后，内网已经与公网互联。  
