...
点击左侧导航 `IPv6` 菜单的子菜单 `DHCP Client` ，点击 ` +New ` 按钮。  
...
点击 Winbox 左侧导航 `IPv6` 菜单的子菜单 `Addresses` ，点击 ` +New ` 按钮。  
...
回到 Winbox ，再次点击 ` +New ` 按钮，方便起见，本文使用 `fdac::/64` 作为演示。  
