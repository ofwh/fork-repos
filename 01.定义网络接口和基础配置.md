...
点击 MAC 地址来登录 RouterOS，默认账户 `admin` ，默认密码为空。  
...
对于未被授权的 RouterOS 将处于“试用”状态，试用时长为 24 小时，且带宽被限制在 `1Mbps` 。  

点击 Winbox 左侧导航的 `Interfaces` ：

![编辑接口](img/p01/Interface.png)

检查网口，此处显示的网口数量及顺序应与 RouterOS 虚拟机中的设置保持一致。
...
首先设置网桥的 IPv4 地址，点击左侧导航 `IP` 菜单的子菜单 `Addresses` ，点击 ` + ` 按钮：
...
先创建 DHCP 地址池，点击左侧导航 `IP` 菜单的子菜单 `Pool` ，点击 ` + ` 按钮：  
...
接下来创建 DHCP 服务器， 点击左侧导航 `IP` 菜单的子菜单 `DHCP Server` ，点击 ` + ` 按钮：
...
接下来创建 DHCP 服务器网络参数，切换到 `Networks` 选项卡，点击 ` + ` 按钮：
