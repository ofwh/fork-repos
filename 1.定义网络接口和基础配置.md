...
![PPPoE时移除网口1DHCP](img/p1/ether1_remove_auto_dhcp.png)
...
在 `Addresses` 处填写的地址段为 `172.16.1.101-172.16.1.150` ，表示该地址池有 50 个可用 IPv4 地址；我预留了一部分地址用于内网设备的静态 IP 设置。   

对于内网设备较多的情况下，可以酌情加大该地址池的空间；由于子网掩码为 `/24` 即 `255.255.255.0` ，因此该地址池最大的地址空间为 `172.16.1.2-172.16.1.254` 。   
