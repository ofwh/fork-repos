...
如果需要禁用某个定时器，可以鼠标 **单击** 选中定时器，点击 `Disable` 按钮进行禁用：
...
根据所使用的 RouterOS 版本不同，从 [Fox_ROS_Native_Sys_Upgrade_Worker.conf](./src/Fox_ROS_Native_Sys_Upgrade_Worker.conf) 和 [Fox_ROS_CHR_Sys_Upgrade_Worker.conf](./src/Fox_ROS_CHR_Sys_Upgrade_Worker.conf) 两个更新脚本中选择一个，并复制其脚本内容。  

鼠标 **双击** `sys_upgrade_worker` 