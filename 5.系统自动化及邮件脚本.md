...
|名称|开始时间|执行周期|说明|
|--|--|--|--|
|system_log_timer|00:00:00|40m|每40分钟执行一次系统日志触发器|
|system_res_timer|00:05:00|6h|每6小时执行一次系统资源触发器|
|system_upgrade_timer|02:35:00|1d|每天执行一次系统更新触发器|
...
复制 [Fox_ROS_Email_Log_Worker.conf](./src/Fox_ROS_Email_Log_Worker.conf) 中的脚本内容，并使用文本编辑器工具对脚本中 **有关邮箱部分** 内容进行修改：
...
如果实际网络环境中没有使用 PPPoE 拨号，则需要额外修改 **有关外网接口部分** 内容：
...
鼠标 **双击** 系统日志邮件脚本 `email_res_worker` ，进入脚本配置界面，在 `Source` 处粘贴脚本内容：
