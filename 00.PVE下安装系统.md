...
## 下载 RouterOS 镜像文件
$ curl -LR -O https://download.mikrotik.com/routeros/7.21.4/chr-7.21.4.img.zip
```
...
## 计算文件 hash
$ sha256sum chr-7.21.4.img.zip

#### 文件 hash 示例输出
29a43395447d88991ad72d09a56247b568b239919eacaacb44581871fd462379  chr-7.21.4.img.zip
```
...
```bash
## 将 img 镜像导入虚拟机中
$ qm importdisk 101 chr-7.21.4.img local-lvm
...
此时，磁盘大小为 `128` MiB ，为了方便后续使用，可通过 PVE `qm` 命令直接将该磁盘调整至目标容量。  
...
或选中该磁盘，然后点击 `磁盘操作` 的 `调整大小` ：

![磁盘扩容-2](img/p00/ros_hd_resize.jpeg)

本文作为演示，仅增加 `1.875` GiB 的磁盘空间。  
