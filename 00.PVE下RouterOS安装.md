...
```bash
# 将 img 镜像导入虚拟机中
qm importdisk 101 chr-7.8.img local-lvm
...
在弹出的对话框中，确认 `IO thread` 选项为 **勾选** 状态，并点击 “添加” 按钮。
...
根据需要，扩容一定量的磁盘空间；我作为演示，仅增加 `1GiB` 的磁盘空间，并点击 “调整磁盘大小” 按钮：
...
`启动延时` 为 `5` ，表示该虚拟机在 PVE 启动完成后，延迟 5 秒自行启动。  
...
RouterOS 的默认账户为 `admin` ，无需密码即可登陆。  
