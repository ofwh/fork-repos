...
根据实际测试，`Queue Tree` 的处理性与 RouterOS 网卡的 `Multiqueue` 有一定关系（参考：[0.PVE下RouterOS安装#17网络](./0.PVE下RouterOS安装.md#17网络)）。   

当 **未设置** 网卡 `Multiqueue` 参数时，`Queue Tree` 在带宽测速时只会使用 4 个 CPU 核心的 **1** 个核心，`Simple Queues` 情况类似。

因此，在未设置网卡 `Multiqueue` 且使用了 `Queue Tree` 或 `Simple Queues` 但无法跑满 QoS 限速的情况下，说明当前设备的单核性能不足，建议开启 RouterOS 网卡的 `Multiqueue` （虽然开启 `Multiqueue` 后会略微降低 QoS 效果），再酌情设置 `Fasttrack` 、 `Simple Queues` 或 `Queue Tree` ，将性能影响降至最低。  
