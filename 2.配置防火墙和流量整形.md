...
RouterOS 的流量整形设置中，使用了在 Linux 下比较流行的 `CAKE` 算法。经过我的测试， `CAKE` 算法相比经典的 `Fq_CoDel` 算法能更好的控制缓冲区肿胀（Bufferbloat）。  
...
当 **未设置** 网卡 `Multiqueue` 参数时， `Queue Tree` 在带宽测速时只会使用 CPU 的 **单个核心** （推测 `Simple Queues` 也会有同样现象）。  

 **因此，如果设备单核处理性能不足，建议开启 RouterOS 网卡的 `Multiqueue` ，并酌情考虑 `Fasttrack` 、 `Simple Queues` 、 `Queue Tree` 带来的性能影响。**    
