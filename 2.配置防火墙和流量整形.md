...
如果这段代码不好复制，请查阅我 Gitee 上的同步更新仓库，里面有单独文件 [Fox_ROS_Define_Interfaces.conf](./src/Fox_ROS_Define_Interfaces.conf) 可以进行拷贝。  

在复制粘贴命令到终端工具并执行时，有时候会出现 **光标** 停留在全部命令的 **“最后一行”** 的情况：

![光标停留在最后一行](img/p2/cli_last_line.png)

这种情况下，命令的 **“最后一行”** 其实并未执行，我们需要手动按下 **“回车”** 键（多按几次当然也可以），以保证命令的全部执行：
