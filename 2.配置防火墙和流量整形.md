...
经过研究官方文档发现，还有一种流量整形的配置方法可以与 `Fasttrack` 功能搭配使用，那就是配置 RouterOS 的“队列树” `Queue Tree` 。  
...
但需要注意的是，在测试过程中发现， `Simple Queues` 可以有效利用 CPU 的 **多个核心** ，而 `Queue Tree` 更多的使用 CPU 的 **单个核心** 。  

 **因此，如果设备单核处理性能不足时，还是建议关闭 `Fasttrack` 并启用 `Simple Queues` 。**    
