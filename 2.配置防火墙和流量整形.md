...
当 **未设置** 网卡 `Multiqueue` 参数时，`Queue Tree` 在带宽测速时只会使用 4 个 CPU 核心的 **1个核心** ，`Simple Queues` 情况类似。

因此，如果设备单核处理性能不足，建议开启 RouterOS 网卡的 `Multiqueue` （虽然开启 `Multiqueue` 后会略微降低 QoS 效果），再酌情考虑 `Fasttrack` 、 `Simple Queues` 、 `Queue Tree` 带来的性能影响。  
