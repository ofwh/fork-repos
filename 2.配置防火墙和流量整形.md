...
```bash
/queue type
add cake-diffserv=besteffort cake-flowmode=triple-isolate cake-memlimit=32.0MiB cake-rtt=60ms cake-overhead-scheme=ethernet cake-nat=yes kind=cake name=cake_total
...
/queue simple
add comment="qosconf: simple queue with cake" bucket-size=0/0 max-limit=500M/50M name=cake_qos queue=cake_rx/cake_tx total-queue=cake_total target=pppoe-out1
```
...
/queue tree
add comment="qosconf: download queue with cake" bucket-size=0 max-limit=500M name=cake_download packet-mark=no-mark parent=bridge1 queue=cake_rx
add comment="qosconf: upload queue with cake" bucket-size=0 max-limit=50M name=cake_upload packet-mark=no-mark parent=pppoe-out1 queue=cake_tx
```
