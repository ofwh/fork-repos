...
防火墙配置命令中的部分 IPv6 地址，需要根据实际情况进行调整。  

例如，请根据实际情况修改 `address-list` 表中的 `local_dns_ipv6` 参数，内网 DNS 服务器的 IPv6 演示地址为 `fdac::2` 、 `fdac::3` 。  

如果没有内网 DNS 服务器，并移除了 `local_dns_ipv6` 地址列表，则需同步移除 `nat` 表中以下 `2` 条防火墙条目：  
